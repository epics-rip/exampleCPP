     ...
     shared_vector<const string> const &channelNames)
{
    ...
    for(int i = 0; i< numiter; i+= 1) {
        ...
        for(size_t j=0; j<num ; ++j) {
            try {
                Status status = pvaClientPuts[j]->waitGet();
                if(!status.isOK()) {
                    cout << "i=" << i << " channel=" << channelNames[j]
            ...
            PvaClientPutDataPtr data = pvaClientPuts[j]->getData();
...
int main(int argc,char *argv[])
{
    ...
    cout << "_____pvaClientPutputPutget starting_______\n";
    try {
        ...
        cout << "_____pvaClientPutputPutget done_______\n";
    } catch (std::exception& e) {
