...
int main(int argc,char *argv[])
{
    ...
    try {
        ...
        clientPut->get();
        string result = clientPut->getData()->getString();
        cout << "result " << result << "\n";
        pva->channel(channelName,provider,2.0)->putString(value,request);
        cout << "result short way " << pva->channel(channelName,provider,2.0)->getString(request) << "\n";
